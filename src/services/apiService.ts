...
export const apiService = {
  ...
  getBookedSlots: async (providerId: string, date: string): Promise<string[]> => {
    try {
      const response = await fetch(`${API_URL}/appointments/booked-slots?providerId=${providerId}&date=${date}`, {
        credentials: 'include',
      });
  ...
  getPaymentByAppointmentId: async (appointmentId: string): Promise<Payment> => {
    try {
      const response = await fetch(`${API_URL}/appointments/${appointmentId}/payment`, {
        credentials: 'include',
      });
